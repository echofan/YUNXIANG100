<template>
  <view-box>
    ...
    <div class="main password-page" v-show="showSetPasswordPage">
      ...
      <div>
        <button class="next" @click="changePassword()">下一步</button>
      </div>
    </div>
  </view-box>
</template>
...
export default {
  ...
  methods: {
    ...
    getVerifyCode: function () {
      ...
      this.$http.post('member/sendSms', { loginname: this.params.loginname, type: 'reset' })
        .then(response => {
    ...
    changePassword: function () {
      ...
      this.$http.post('member/resetPassword', null, { params: this.params })
        .then(response => {
          return response.data
        }).then(response => {
          if (response.code === 0) {
