<template>
  <div style="flex:1;overflow: auto; position: relative;background:#eee;height:100%;">
    ...
    <div class="operate">
      <div>
        <button class="btn save" @click="editAddress">修改保存</button>
      </div>
      <!-- <div>
...
export default {
  ...
  methods: {
    ...
    getAddress: function () {
      this.$http.post('member/addressDetail', null, {
        params: {
        ...
        .then(response => {
          ...
          if (response.code === 0) {
            response.data.areaValue = response.data.area.split(' ')
    ...
    editAddress: function () {
      ...
      this.$http.post('member/editAddress', null, {
        params: {
          memberId: this.$cookie.get('memberId'),
          token: this.$cookie.get('token'),
          addressId: this.$route.params.id,
