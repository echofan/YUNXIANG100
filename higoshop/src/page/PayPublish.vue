...
export default {
  ...
  methods: {
        ...
        WeixinJSBridge.invoke('getBrandWCPayRequest', {
          ...
          function (res) {
            if (res.err_msg === 'get_brand_wcpay_request:ok') {
              that.$router.push('/user/mypublishid')
    ...
    pay: function () {
      var para = {
        ...
        activeCodeId: this.$route.query['activeCodeId'],
        money: this.id.totalPrice
      }
      this.$http.post('activationCode/wxpay', null, {
        params: {
  ...
  created: function () {
    this.$http.post('activationCode/activeCodePayDetail', null, {
      params: {
        memberId: this.$cookie.get('memberId'),
        token: this.$cookie.get('token'),
        activeCodeId: this.$route.query.activeCodeId
