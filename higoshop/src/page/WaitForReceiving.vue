  ...
  <div class="page">
      ...
      <flexbox :gutter="0" class="order-list" v-if="orderList.length > 0">
        <flexbox-item class="order-item" v-for="order in orderList" :key="order.id">
          <div class="simple-info" @click="show(order)">
            ...
            &nbsp; &nbsp;
            <span v-text="dateFormat(new Date(order.consignTime*1000),'YYYY-MM-DD HH.mm')"></span>
          </div>
          <div class="detail" v-show="!order.showDetail">
            ...
            <div class="commodity-total">
              <flexbox :gutter="0">
                <flexbox-item :span="1/10">
                  <input type="checkbox" :value="order.id" v-model="params.cartIds">
                </flexbox-item>
                <flexbox-item :span="9/10" style="text-align:right;">
            ...
            <div class="express">
              <span>物流</span>
              <span v-text="order.courierFirm">顺丰快递</span>
              <span>单号：{{order.courierNum}}</span>
            </div>
            <div class="order-status clear">
              ...
              <div class="float-left">
                <img src="../assets/20170727221222.png" alt="">
                <p>配送中</p>
              </div>
              <div class="float-left">
                <img src="../assets/2.png" alt="">
    ...
    <div class="total" v-if="orderList.length > 0">
      <flexbox :gutter="0" align="stretch">
        ...
        <flexbox-item :span="3">
          <button class="btn-submit" @click="toReceiving">收货</button>
        </flexbox-item>
      </flexbox>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    selectAll: function (e) {
      if (e.target.checked) {
        for (var i = 0; i < this.orderList.length; i++) {
          if (this.params.cartIds.indexOf(this.orderList[i].id) === -1) {
            this.params.cartIds.push(this.orderList[i].id)
          }
        }
      } else {
        this.params.cartIds = []
    ...
    toReceiving: function () {
      if (this.params.cartIds.length <= 0) {
        this.$vux.toast.show({ type: 'warn', text: '请选择订单' })
        return
      }
      this.$http.post('ordergoods/receiveGoodsOrder', null, {
        params: {
          memberId: this.$cookie.get('memberId'),
          token: this.$cookie.get('token'),
          'orderGoodsId[]': this.params.cartIds.join(',')
        }
      })
        .then(response => {
          return response.data
          ...
          if (response.code === 0) {
            ...
            for (var i = 0; i < this.orderList.length; i++) {
              var index = this.params.cartIds.indexOf(this.orderList[i].id)
              if (index !== -1) {
            ...
            this.params.cartIds = []
            this.$vux.toast.show({ text: response.msg })
          } else {
            this.$vux.toast.show({ type: 'warn', text: response.msg })
          }
        })
