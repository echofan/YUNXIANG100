<template>
  <div style="flex:1;overflow: auto; position: relative;height:100%;">
    ...
    <div class="table-wrap">
      <table class="table">
        <tbody v-if="subNoActiveCodeList.length>0">
          <tr v-for="l in subNoActiveCodeList" :key="l.id">
            <td>
...
export default {
  ...
  methods: {
    ...
    getSubNoActiveCode: function () {
      this.$http.post('member/getSubNoActiveCode', null, {
        params: {
          memberId: this.$cookie.get('memberId'),
          token: this.$cookie.get('token')
        }
        ...
        .then(response => {
          ...
          if (response.code === 0) {
            this.subNoActiveCodeList = response.data
            console.info(this.subNoActiveCodeList.length)
          } else {
            this.$vux.toast.show({ type: 'warn', text: response.msg })
          }
        })
